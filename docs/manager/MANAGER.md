...
6. **No invented facts about the environment.** Claims about Claude Code features, library APIs, or upstream behaviour of AzerothCore/WoWee must come from docs, code reading, or a survey document — never from memory alone. If unverified, verify or mark as assumption in the task log.
   **6b. The caveat travels with the claim** (Ludwig, 2026-09-05): *no contract or spec states an environmental fact without either a verification command shown or an inline caveat marking it unverified.* The task log is not sufficient — a contract is read by the next implementer **without** its log, so a caveat that lives only in a "could not verify" section at the bottom of a task file does not reach the person the rule exists to protect. Mark it where the claim is made, name what would confirm it and who confirms it. The caveat marks the **basis** of the rule, never softens the rule itself: "this is required; this is why we believe it; that belief is untested here" — not "this might be required". Found in task 025's review, where three honestly-logged unverified claims (minisign's byte layout, Windows reserved device names, GitHub Pages behaviour) were each stated as flat fact in the contract text a future implementer would build from.
   **6c. 6b covers what you cannot check; it never substitutes for the sub-minute check you can** (Ludwig, 2026-09-05). Task 025's *second* review round found two claims that were not folklore and not unverifiable — they were simply wrong, and each took under a minute to falsify with a tool already installed. `signature-format.md` said a minisign public key's 16-hex `key_id` was the blob's 8 key-id bytes "in the same byte order"; it is the **reverse**, confirmed against this project's own production key (raw `C48C7A6A59305B6E`, reversed `6E5B30596A7A8CC4`, which is the real `key_id`). Because the same document's rule 6 rejects a signature whose `key_id` does not match those bytes, **a verifier built from that text would have rejected every correctly signed artefact this platform ever produces** — found the same morning task 008's signing secret was loaded. The second: `archive-layout.md` recommended `git archive` and claimed it names the archive root after the ref; plain `git archive` produces no root at all, which that same document elsewhere rejects as malformed. When a document makes a claim about a tool's behaviour and the tool is on this machine, **run it**. A caveat where a command was available is not caution, it is a missing check wearing caution's clothes.
...
- **The token guard (inviolable; CLAUDE.md rule 0).** Ludwig's Claude Max subscription is shared
  ...
  counts as above 90 %:** stop and ask Ludwig.
  **Interim rule in force from 2026-09-05 until task 031 lands** (Ludwig): `~/.claude/usage-snapshot.json`
  and every tool reading it — `token-guard-check.sh`, `checkpoint.sh` — are **advisory only**. Ludwig's
  stated figure is authoritative; **absent a fresh figure from him, prefer UNKNOWN over the file**, and
  UNKNOWN halts. The file is written by *every* running session, so an idle one, or one started by an
  agent's own test, rewrites frozen values under a fresh timestamp. Observed again on 2026-09-05: the
  file read 40 % while Ludwig's HUD read **55 %** minutes later — a 15-point gap **in the unsafe
  direction**, during a session in which task 023's round had started real `claude` sessions of its own. This rule outranks mission progress, open tasks and
